...
<script>
  ...
  function init(){  //초기화. 상품 주민 페이지가 표시되면 주문자의 휴대폰 번호화 유선 전화번호를 셀렉트 박스에 표시
	...
	function restore_all() { //다른거 눌렀다 다시 기본배송지 누르면, 기본배송지로 복원됨. 
		var e_receiver_name = document.getElementById("receiver_name");
		var e_hp1 = document.getElementById("hp1");
		var e_hp2 = document.getElementById("hp2");
		var e_hp3 = document.getElementById("hp3");
...
function fn_pay_phone(){  //결제 정보에서 핸드폰 결제 체크 
...
function fn_pay_card(){  //결제 정보에서 신용카드 체크 
	var e_card=document.getElementById("tr_pay_card");
	var e_phone=document.getElementById("tr_pay_phone");
...
function fn_show_order_detail(){ //결제하기 버튼 누르면  (최종 결제하기 버튼 전에, orderEachGoods 창에서 )
	goods_id="";
	goods_title="";
	...
	if(h_goods_id.length <2 ||h_goods_id.length==null){ //동일 id hidden태그 두개 이상이면 배열이므로 
		goods_id+=h_goods_id.value;       //주문 상품 두 개 이상일 때랑 아닐 때 구분 
	}else{
	...
	for(var i=0; i<r_delivery_method.length;i++){ //delivery_method - 배송방법 (r은 변수로 가져온거 )
	...
	for(var i=0; i<r_gift_wrapping.length;i++){  //wrapping - 선물포장 
	...
	var r_pay_method  =  frm.pay_method;    //pay_method- 결제방법 
	...
	for(var i=0; i<r_pay_method.length;i++){
	  if(r_pay_method[i].checked==true){
		  ...
		  if(pay_method=="신용카드"){
			...
			card_com_name=i_card_com_name.value;    //input의 value 값 받아서 저장해줌 
	...
	var i_hp1=document.getElementById("hp1");  //hp는 주문고객의 핸드폰 번호 000-000-000 에서 -를 기점으로 세등분한 각 부분 
	var i_hp2=document.getElementById("hp2");
	var i_hp3=document.getElementById("hp3");
	
	var i_tel1=document.getElementById("tel1"); //tel 은 유선전화의 각 부분 
...
//	alert("총주문 금액:"+total_order_goods_price);
	...
	delivery_message=i_delivery_message.value;  // 배송메시지 
	
	var p_order_goods_id=document.getElementById("p_order_goods_id");  //배송 상품 등 각 요소들의 <p>태그 id들
	...
	p_order_goods_id.innerHTML=goods_id;    //위에서 변수로 만들어준 값들을 가지고 실제 p태그의 내용물을 바꿔줌 (주의  -숨긴처리된 div 부분의 <p>태그)
	...
	imagePopup('open');  //imagePopup 함수 실행 
}

function fn_process_pay_order(){  //'최종 ' 결제하기 버튼 누르면 
    ...
    //추가
    i_receiver_name.type="hidden";
    i_receiver_hp1.type="hidden";
    i_receiver_hp2.type="hidden";
    i_receiver_hp3.type="hidden";
    i_receiver_tel1.type="hidden";
    i_receiver_tel2.type="hidden";
    i_receiver_tel3.type="hidden";
    i_delivery_address.type="hidden";
    i_delivery_message.type="hidden";
    i_delivery_method.type="hidden";
    i_gift_wrapping.type="hidden";
    i_pay_method.type="hidden";
    i_card_com_name.type="hidden";
    i_card_pay_month.type="hidden";
    i_pay_orderer_hp_num.type="hidden";
