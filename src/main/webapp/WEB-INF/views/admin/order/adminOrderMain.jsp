...
function search_order_history(search_period){	
	...
	var i_command = document.createElement("input"); 이거 왜? 잘못쓴듯
    ...
    */
    ...
    i_fixedSearchPeriod.type = "hidden"; //태그 안보이게 
...
function fn_detail_search(){
	...
	//추가 input 값을 hidden으로 해서, 버튼 눌렀을 때 안 보이게 하기 
    i_command.type="hidden";
	i_beginDate.type="hidden";
	i_endDate.type="hidden";
	i_search_type.type="hidden";
	i_search_word.type="hidden";
    ...
    document.body.appendChild(formObj); 
    formObj.method="post";
    //formObj.action="${contextPath}/admin/order/detailOrder.do"; //그냥 adminOrderMain.do 로 보내는걸로 수정하기 
    formObj.action = "${contextPath}/admin/order/adminOrderMain.do";
    formObj.submit();
    //alert("submit");
...
//주문 삭제 기능 추가
function fn_delete_order(order_id,url) {
	...
	orderIdInput.setAttribute("value", order_id);
			...
			<tbody>
				...
				<tr>
				  <td>
					...
					-->
					<!-- beginYear -> currentBeginYear로 수정 -->
					<select name="beginYear" disabled >
					 <c:forEach   var="i" begin="0" end="5">
					      <c:choose>
					        <c:when test="${currentBeginYear==currentBeginYear-i }">
					          <option value="${currentBeginYear-i }" selected>${currentBeginYear-i  }</option>
					        </c:when>
					        <c:otherwise>
					          <option value="${currentBeginYear-i }">${currentBeginYear-i }</option>
					        </c:otherwise>
					      </c:choose>
					    </c:forEach>
					</select>년 					
					...
					-->
					<!-- endYear -> currentEndYear로 수정 -->
					<select name="endYear" disabled >
					 <c:forEach   var="i" begin="0" end="5">
					      <c:choose>
					        <c:when test="${currentEndYear==currentEndYear-i }">
					          <option value="${currentEndYear-i }" selected>${currentEndYear-i  }</option>
					        </c:when>
					        <c:otherwise>
					          <option value="${currentEndYear-i }" >${currentEndYear-i }</option>
					        </c:otherwise>
					      </c:choose>
					    </c:forEach>
					</select>년 					
          ...
          <c:when test="${item.order_id != pre_order_id }">  
            <c:choose>
              ...
              <c:otherwise>
			    ...
			    <td>
			    		 <!-- 삭제 버튼 추가 -->			     
			          <input type="button" value="삭제" onClick="fn_delete_order('${item.order_id}','${contextPath}/admin/order/removeOrder.do')" />
			    </td>
			</tr>
		</c:when>
		</c:choose>	
        ...
        <c:choose>
             ...
             <c:choose>  
                  <c:when test="${section==lastSec }"> 
		                <c:forEach var="page" begin="1" end="${end }" step="1">
		                    <c:if test="${section > 1 && page == 1 }">  
		                       <a class="no-uline" href="${contextPath }/admin/order/adminOrderMain.do?section=${section-1}&pageNum=1">pre</a>              
		                    </c:if>		                   
		                    <c:choose>
		                       <c:when test="${page==pageNum }">
		                          <a class="sel-page" href="${contextPath }/admin/order/adminOrderMain.do?section=${section}&pageNum=${page}">${(section-1)*10+page}</a>
		                       </c:when>
		                       <c:otherwise>
		                          <a class="no-uline" href="${contextPath }/admin/order/adminOrderMain.do?section=${section}&pageNum=${page}">${(section-1)*10+page}</a>
		                       </c:otherwise>
		                    </c:choose>
		                    
		                    <c:if test="${page==10 }">
		                       <a class="no-uline" href="${contextPath }/admin/order/adminOrderMain.do?section=${section+1}&pageNum=1">next</a>
		                    </c:if>
		                </c:forEach>
		             </c:when>
             ...
             <c:when test="${totArticles == 100 }">
                <c:forEach var="page" begin="1" end="10" step="1">              
                  <c:choose>
                     <c:when test="${page==pageNum }">
                         <a class="sel-page" href="${contextPath }/admin/order/adminOrderMain.do?section=${section}&pageNum=${page}">${page }</a>
                     </c:when>
                     <c:otherwise>
                         <a class="no-uline" href="${contextPath }/admin/order/adminOrderMain.do?section=${section}&pageNum=${page}">${page }</a>
                     </c:otherwise>
                  </c:choose>                 
                </c:forEach>                
             </c:when>
