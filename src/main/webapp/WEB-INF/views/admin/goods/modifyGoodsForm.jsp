...
function fn_modify_goods(goods_id, attribute){
  ...
  // 디버깅- 작동안될때 location.reload() 중간중간 넣어보기 
  function modifyImageFile(fileId,goods_id, image_id,fileType){ //
     //alert(fileId);
      ...
      //location.reload();
      formData.append("goods_id", goods_id); //이건 이미지당 모두 같을거임.
      formData.append("image_id", image_id); //image_id -> 각 이미지마다 다를것. 분류해주는 작업이 필요.  
      formData.append("fileType", fileType);  //이거도 각 이미지마다 다를것. 
  ...
  function addNewImageFile(fileId,goods_id, fileType){
	   //  alert(fileId);
	      ...
	      // fileId - fn_addFile()에서 cnt와 함께 생성돼서 인자로 들어오는 값.
					...
					<tr>
			         ...
			         <c:otherwise>
			           <tr  id="${itemNum.count-1}">   
						...
						<td>
						        <!-- name = "detail_image" 에서  detail_image${itemNum.index}로 바꾼다.-->
						        <!--  id 값도 이런식으로 할당해주어야한다. id="detail_image"에서 detail_image${itemNum.index} 로  - 근데 이걸 바꾸면 fn_addFile 함수도 수정해야함 -->
						        <!-- 참고 - 처음 detail_image${itemNum.index} 는 detail_image1임 (0번째는 main_image 에서 가져갔으므로)  -->
							<input type="file" name="detail_image${itemNum.index }"  id="detail_image${itemNum.index }"   onchange="readURL(this,'preview${itemNum.count}');" />
							<%-- <input type="text" id="image_id${itemNum.count }"  value="${item.fileName }" disabled  /> --%>
							<input type="hidden"  name="image_id" value="${item.image_id }"  />
							<br>
							<!-- 각 forEach문마다  id, name 값을 유동적으로  만들어주기 위해 변수 추가 -->
							<!-- 사실상 각 forEach문에서 id 값과 name 값이 같기 때문에 변수는 하나만 만들면 된다. -->							
							<c:set var="detail_image" value="detail_image${itemNum.index }"/>
						...
						<td>
						 ...
						 <td>
						 <!-- 수정  'detail_image' 를 위에서  c:set으로 설정한 변수 detail_image로 (${detail_image}-->
