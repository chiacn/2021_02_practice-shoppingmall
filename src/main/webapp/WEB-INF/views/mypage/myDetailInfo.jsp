...
function fn_modify_member_info(attribute){ //attribute -수정하는 데이터가 무엇인지에 따라 인자로 들어오는 값이 달라짐 
	
	var value; //수정된 값을 ajax에 인자로 넣기 위해 value 값 설정
	
	// alert(member_id);
	// alert("mod_type:"+mod_type);
		var frm_mod_member=document.frm_mod_member; //form 네임
		
		if(attribute=='member_pw'){   //attribute - 인자로 들어오는 값. 예를들어 여기선 비밀번호(member_pw)를 수정하고 수정버튼을 눌렀을 때.
			value=frm_mod_member.member_pw.value; // frm_mod_member의 member_pw 의 값으로 value 값을 바꿈 
			...
			for(var i=0; member_gender.length;i++){
			 	if(member_gender[i].checked){   //checkbox
					value=member_gender[i].value;
					break;
				} 
			}
			
		}else if(attribute=='member_birth'){ //생년월일 수정 
			...
			for(var i=0; member_birth_y.length;i++){ //for 돌려서 selected 되어 있는 값의 value를 value_y로 저장  
			...
			value=+value_y+","+value_m+","+value_d+","+value_gn; //각 년월일 취합 ->value 값으로 합치기 
		...
		$.ajax({
			...
			data : {   //------------>MyPageController의 modifyMyInfo 메서드로 보내지는 값
				attribute:attribute,   
				value:value,
			},
		...
		<table>
			<tbody>
				...
				<tr class="dot_line">
					...
					<!-- 회원 성별이 남성이면 남성에 체크되어있게, 반대면 반대에 체크되어있도록  -->
					  <c:choose >  
				...
				<tr class="dot_line">
					...
					<td>
					<!-- 설명 : forEach로 돌아가는 1~100 중 회원의 생년월일 년도(member_birth_y와 일치할 경우
					    selected 된 상태로. 아닌 경우 그냥(selected 안 된) option으로 표기 
					    <option value="${ 1920+i}" >${ 1920+i} </option> (value값은 value 값, 태그 사이는 표시되는 값
					    -->
